...
tostring hoursperweek, gen(hours_string)
	* Notice that the hours_string variable is of type "str3"
	
* Changing non-strings to strings is kind of rare; Stata prefers non-string data.
		* Besides being more useful for analysis, it takes up less memory, so Stata will run faster.
...
* Changing strings to numeric data is more common, usually because you're importing data from something that kept it as strings.
		* You generally want to make a new variable with the gen option, unless you really don't want the string variable.
		
destring hours_string, gen(hours_string_destrung)	

* Stata only likes to do this for string data that already contains numeric characters.
* destring isn't useful for making dummy variables or categorical variables.

destring country1 // Wants either a gen or replace option.

destring country1, replace // Won't run since it's not numeric data.

* You would have to generate dummy variables through either gen and replace or the tab option.

tab pronouns, gen(gender_dummy)	// More on this in a minute!
...
tab age in 5 // Shows age of 5th observation in dataset

drop in 5 // Drops 5th observation (very rarely should you ever do this)
* Notice that there is no variable between drop and in!
...
sort age // Sorts in ascending order of age
...
by poverty_bin: summarize ftotinc 
...
egen age_5_bins = cut(age), group(5) // This creates a categorical variable with 5 levels, corresponding to the quintiles of the age variable

tab age_5_bins

egen mean_age_minor = mean(age) if age < 18 // You can combine if and in with egen too.

egen mean_age_adult = mean(age) if age >= 18

gen mean_age_comb = .
replace mean_age_comb = mean_age_minor if age < 18
replace mean_age_comb = mean_age_adult if age >= 18
...
xtile age_5_bins_again = age, n(5)

tab age_5_bins_again

* tab can also be used to generate dummy variables, as we just saw.
tab age_5_bins_again, gen(age_quint)

* You can use egen in a lot of complex ways. To see all the functions availabe with egen check out its help file.

help egen

* Note: Somewhat surprisingly, egen doesn't do log transformations. For those just use gen.

gen log_age = log(age)

tab log_age, m // Just remember that if Stata has an error with the math, it'll create missing values. log(0) gives an "."
...
pwcorr sex age marst educ inctot ftotinc poverty

	* What does it mean that poverty is positively correlated with income here?
	* Is that what we would expect?
	
pwcorr sex age marst educ inctot ftotinc poverty poverty_bin	
	
	* We can also run correlations on sub-samples:
	
pwcorr sex age marst inctot if poverty_bin == 1 // Just correlations for those below the poverty line.	
		
		* Just don't include the "if" variable in the correlation test.
...
ttest female == 0.5 // We choose 0.5 on the assumption that under the null hypothesis, the mix would be 50-50.
...
ttest inctot, by(female)	
	...
	* Output is similar, but now we have two variable means and the difference between them.

* Important note: If the variance of the two groups is dissimilar, include the option "unequal"

	tabstat inctot if female == 0, s(var)
	tabstat inctot if female == 1, s(var)
	
	* Are these variances "equal" or "unequal"? Hard to say.
	
	ttest inctot, by(female) unequal // But the outcome is roughly the same as before, so we'll assume it's equal.
...
ttest ftotinc == inctot
		...
		* As expected, the average family income is higher than individual income.
		* The t-stat is huge and the p-value is 0.0000, so it's a very very significant difference
...
tab marst female, chi2
	...
	* As this is below the standard 0.05 used for 95% confidence,
		* we conclude that the two variables marital status and female are
			* not independent.
	* Therefore there is some relationship between marital status and gender.
